...
class _LiveTripTrackingScreenState extends State<LiveTripTrackingScreen> {
  ...
  LatLng? _myLocation;   // passenger
  LatLng? _driverPos;    // driver (simulated / from polling)
  MapRoute? _route;
  ...
  Future<void> _init() async {
    ...
    // Driver starts offset from passenger; will update via polling
    final driverPos =
        LatLng(myPos.latitude + 0.008, myPos.longitude + 0.006);
  ...
  // ── Poll trip status + driver position from server ────────────────────────
  Future<void> _pollTrip() async {
    try {
      ...
      // Update driver marker if server provides coords
      final driverLat =
          double.tryParse(trip['driver_lat']?.toString() ?? '');
      final driverLng =
          double.tryParse(trip['driver_lng']?.toString() ?? '');
      if (driverLat != null && driverLng != null) {
        final newDriverPos = LatLng(driverLat, driverLng);
        setState(() => _driverPos = newDriverPos);
        _updateDriverMarker(newDriverPos);

        // Refresh road route with updated driver position
    ...
    setState(() {
      _markers = {
        ...
        Marker(
          ...
          infoWindow:
              InfoWindow(title: widget.driverName, snippet: 'Driver'),
        ),
      };
    });
  }
  ...
  Future<void> _checkIfCompleted() async {
    try {
      ...
      if (history.isNotEmpty &&
          history.first['status'] == 'completed') {
        _showCompleted();
        return;
      }
    } catch (_) {}
  ...
  void _showCompleted() => showDialog(
        context: context,
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
              'You have arrived. Thanks for riding with TodaGo!',
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              // Route info box (mirrors destination_picker style)
              Container(
