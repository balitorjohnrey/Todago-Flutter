...
class MapService {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (status == 'OK') {
          ...
          _log(
              'Places',
              'REQUEST_DENIED — check that Places API is enabled '
  ...
  static Future<List<PlaceSuggestion>> _searchPhoton(
    ...
    try {
      ...
      _log('Photon',
          'Body preview: ${res.body.substring(0, res.body.length.clamp(0, 400))}');
      ...
      for (final f in features) {
        ...
        if (mainText.isEmpty) {
          _log(
              'Photon', 'Skipping feature with empty mainText — props: $props');
          continue;
        }
        ...
        if (district.isNotEmpty && mainText != district)
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (results != null && results.isNotEmpty) {
          final address =
              results.first['formatted_address'] as String? ?? 'Your Location';
