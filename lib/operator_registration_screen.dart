...
class _OperatorRegistrationScreenState extends State<OperatorRegistrationScreen> {
  ...
  @override
  void dispose() {
    _assocNameCtrl.dispose(); _assocIdCtrl.dispose();
    _ltfrbCtrl.dispose(); _regionCtrl.dispose();
    _contactNameCtrl.dispose(); _emailCtrl.dispose();
    _mobileCtrl.dispose(); _passwordCtrl.dispose();
    _confirmCtrl.dispose(); _totalTricyclesCtrl.dispose();
  ...
  Future<void> _submit() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(children: [
        ...
        Expanded(child: Text('Registration submitted! Pending LTFRB verification.',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _lightField(controller: _assocNameCtrl, hint: 'Davao-Central TODA',
              icon: Icons.groups_rounded),
          const SizedBox(height: 18),
          _lightLabel('Association ID / TODA ID', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _assocIdCtrl, hint: 'e.g., DCC-TODA-001',
              icon: Icons.badge_outlined),
          const SizedBox(height: 18),
          _lightLabel('LTFRB Case Number', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _ltfrbCtrl, hint: 'LTFRB franchise number',
              icon: Icons.description_outlined),
          const SizedBox(height: 18),
          _lightLabel('Region / City', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _regionCtrl, hint: 'Davao City, Davao del Sur',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          _lightField(controller: _contactNameCtrl, hint: 'Full name of authorized officer',
              icon: Icons.person_outline_rounded),
          const SizedBox(height: 18),
          _lightLabel('Operator Email', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _emailCtrl, hint: 'operator@toda.ph',
              icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress),
          const SizedBox(height: 18),
          _lightLabel('Mobile Number', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _mobileCtrl, hint: '+63 912 345 6789',
              icon: Icons.phone_outlined, keyboardType: TextInputType.phone),
          const SizedBox(height: 18),
          _lightLabel('Password', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _passwordCtrl, hint: 'Min. 8 characters',
          ...
          const SizedBox(height: 6),
          _lightField(controller: _confirmCtrl, hint: 'Re-enter your password',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          _lightField(controller: _totalTricyclesCtrl, hint: 'e.g., 402',
              icon: Icons.directions_car_outlined, keyboardType: TextInputType.number),
          const SizedBox(height: 18),
          _lightLabel('Number of Active Drivers'),
          const SizedBox(height: 6),
          _lightField(controller: _activeDriversCtrl, hint: 'e.g., 315',
              icon: Icons.people_outline_rounded, keyboardType: TextInputType.number),
          const SizedBox(height: 18),
          _lightLabel('Primary Service Area', required: true),
          const SizedBox(height: 6),
          _lightField(controller: _serviceAreaCtrl, hint: 'e.g., Davao City CBD, Panabo City',
